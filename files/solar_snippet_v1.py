...
def paste_solar_panel(target_image, target_mask, target_mask_np, cropped_image, cropped_mask, location, bounding_box):
    ...
    if random.random() < 0.2:
        ...
        cropped_image = brightness.enhance(random.uniform(0.8, 1.2))

    # Apply random contrast to the cropped image
    if random.random() < 0.2:
        contrast = ImageEnhance.Contrast(cropped_image)
        cropped_image = contrast.enhance(random.uniform(0.8, 1.2))
...
class ImageProcessor:
    def __init__(self, solar_image_dir, solar_mask_dir):
        self.parent_dir = os.path.dirname(os.getcwd())
        self.solar_image_dir = solar_image_dir
        self.solar_mask_dir = solar_mask_dir
        self.building_image_dir = os.path.join(parent_dir, 'data', 'Munich_rooftops_noPV', 'images')
        self.building_mask_dir = os.path.join(parent_dir, 'data', 'Munich_rooftops_noPV', 'building_masks')

    def process_sample_images(self):
        # Get the solar image and mask filenames
        solar_image_files = sorted(os.listdir(self.solar_image_dir))

        # Get the building image and mask filenames
        building_image_files = sorted(os.listdir(self.building_image_dir))

        # Randomly select one image file from each directory
        solar_image_file = random.choice(solar_image_files)
        building_image_file = random.choice(building_image_files)

        source_image = Image.open(os.path.join(self.solar_image_dir, solar_image_file)).convert("RGB")
        source_mask = Image.open(os.path.join(self.solar_mask_dir, solar_image_file)).convert("L")
        target_image = Image.open(os.path.join(self.building_image_dir, building_image_file)).convert("RGB")
        target_mask = Image.open(os.path.join(self.building_mask_dir, building_image_file)).convert("L")
...
if __name__ == "__main__":
    ...
    image_processor = ImageProcessor(solar_image_dir, solar_mask_dir)
